...
export function ImageStudio({ files, onReset }: { files: File[]; onReset: () => void }) {
  ...
  const run = async () => {
    ...
    try {
      const result = await transformImageBatch(files, { format, maxEdge, quality, rotate, flipHorizontal, flipVertical: false, background: "#ffffff" }, abort.signal, setProgress);
