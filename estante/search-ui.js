...
(function(){
  ...
  $("searchForm").onsubmit=async e=>{e.preventDefault();const q=$("searchInput").value.trim();if(!q)return;const button=$("searchForm").querySelector("button");button.disabled=true;button.textContent="Buscando…";notify(state.source==="smart"?"Busca inteligente: consultando várias fontes e variações…":"Procurando…",true);try{state.results=await searchMusic(q);state.tab="results";renderList();if(!state.results.length){notify("Não encontrei essa música. Tente também um trecho da letra ou confira a grafia do artista.");return}if(state.source==="smart"){const src=state.searchMeta?.sources?.join(" + ")||"múltiplas fontes";notify(`${state.results.length} resultado${state.results.length===1?"":"s"} · ${src}. Os melhores aparecem primeiro.`,true)}else notify("")}catch(err){state.results=[];renderList();notify(navigator.onLine?`Falha na busca: ${esc(err.message)}`:"Você está offline. O repertório salvo continua funcionando.")}finally{button.disabled=false;button.textContent="Buscar"}};
