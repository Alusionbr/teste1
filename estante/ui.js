...
(function init(){const oldP=load("estante:preferencias",{}),p=load(KEYS.prefs,null)||{source:oldP.fonte,speed:oldP.velocidade,font:oldP.corpo,stage:oldP.palco,keyVag:oldP.chaveVagalume};loadSetlists();state.source=(p.source==="trecho"?"excerpt":p.source)||"lrclib";state.speed=state.speedGlobal=p.speed||18;state.font=p.font||26;state.stage=!!p.stage;state.keyVag=p.keyVag||"";state.keyYT=p.keyYT||"";state.audioDelay=Number(p.audioDelay)||0;state.pedalMode=p.pedalMode==="pedaleira"?"pedaleira":"toque";state.pedalOffered=!!p.pedalOffered;state.naipe=String(p.naipe||"").slice(0,24);state.soMinhaVoz=p.soMinhaVoz!==false;state.installOferecido=!!p.installOferecido;state.ultimoBackup=Number(p.ultimoBackup)||0;document.querySelectorAll(".chip[data-source]").forEach(b=>b.classList.toggle("active",b.dataset.source===state.source));$("searchInput").placeholder=state.source==="excerpt"?"Um trecho da letra":state.source==="lrclib"?"Música, artista ou álbum":"Artista e música";updateControls();updateNetwork();renderList();updateSaveButton();readSharedLink().then(incoming=>{if(incoming)showIncomingSetlist(incoming);else{$("searchInput").focus();guardarNaAbertura()}})})();
