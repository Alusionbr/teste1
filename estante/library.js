 ...
 *
 * Nas pontas o índice era limitado e a música reaberta assim mesmo: apertar ›
 * na última fazia openSong() rodar de novo, parando a rolagem e voltando ao
 * topo. Um toque a mais na pedaleira apagava a sua posição no meio da música.
 * Agora, se não há para onde ir, não faz nada.
 */
function jumpSong(d){
  if(!state.setlist.length)return;
  const i=state.currentIndex<0?(d>0?0:state.setlist.length-1):state.currentIndex+d;
