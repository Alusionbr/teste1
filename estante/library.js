...
function addSong(){if(!state.current)return;const exists=state.setlist.some(x=>x.title===state.current.title&&x.artist===state.current.artist);if(exists)return notify("Essa música já está no repertório.",true);state.setlist.push(storedSong(state.current));save(KEYS.setlist,state.setlist);state.tab="setlist";renderList();updateSaveButton();notify("Adicionada ao repertório.",true)}
