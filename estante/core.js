 ...
 */
const state={results:[],setlist:[],setlists:[],recoveredSetlists:[],trash:[],resume:null,activeSetlistId:"",tab:"results",view:"home",source:"lrclib",current:null,currentIndex:-1,lines:[],lrc:[],scrolling:false,syncing:false,karaoke:false,videoPlaying:false,speed:18,speedGlobal:18,font:32,key:0,capo:0,auto:false,stage:false,theme:"grafite-refinado",keyVag:"",keyYT:"",audioDelay:0,saveState:"saved",pendingSuccessNotice:"",readOnly:false,storageError:"",documentRevision:0,loadToken:0,songAbort:null};
...
async function fetchSafe(url,options={},timeout=12000){const ctrl=new AbortController(),external=options.signal;if(external){if(external.aborted)ctrl.abort();else external.addEventListener("abort",()=>ctrl.abort(),{once:true})}const timer=setTimeout(()=>ctrl.abort(),timeout);try{return await fetch(url,{...options,signal:ctrl.signal})}catch(e){if(e.name==="AbortError")throw Error("A fonte demorou demais para responder. Tente novamente.");throw e}finally{clearTimeout(timer)}}
/*
...
function renderNoticeRaw(msg,ok=false){$("notice").innerHTML=msg?`<div class="notice ${ok?"ok":""}">${msg}</div>`:""}
function notify(msg,ok=false){if(ok&&state.saveState==="saving"){state.pendingSuccessNotice=msg;return}renderNoticeRaw(msg,ok)}
...
async function fetchVagalume(song,options={}){
  ...
  if(song.vagId){try{const id=String(song.vagId).replace(/^l/,"");const r=await fetchRetrying(`${base}&musid=${encodeURIComponent(id)}`,options);if(r.ok)d=await r.json()}catch{}}
  if(!d?.mus?.[0]?.text){
    let r;try{r=await fetchRetrying(`${base}&art=${encodeURIComponent(song.artist)}&mus=${encodeURIComponent(song.title)}`,options)}catch(e){markSource("vagalume",false,e.message);throw sourceError("vagalume","O Vagalume não respondeu a tempo.")}
    if(!r.ok){markSource("vagalume",false,r.status);throw sourceError("vagalume",`O Vagalume está fora do ar agora (erro ${r.status}).`)}
    d=await r.json();
  }
...
async function fetchLrclibSong(song,options={}){
  const qs=new URLSearchParams({track_name:song.title||""});if(song.artist)qs.set("artist_name",song.artist);
  const r=await fetchSafe(`https://lrclib.net/api/search?${qs}`,{headers:LRCLIB_HEADERS,...options});
...
function persistCurrent(fields){
  ...
  const owner=state.current.ownerSetlistId,entryId=state.current.entryId;
  const set=state.setlists.find(x=>x.id===owner),i=set&&set.songs.findIndex(x=>x.entryId===entryId);
  if(!set||i<0)return;
  if(fields){const values={};fields.forEach(f=>values[f]=state.current[f]);updateEntrySetting(owner,entryId,values);saveSetlistsSoon()}
  else{const next=storedSong(state.current);next.ownerSetlistId=owner;next.entryId=entryId;next.contentRevision=(state.current.contentRevision||1)+1;set.songs[i]=next;state.current=next;if(owner===state.activeSetlistId)bindActiveSetlist();saveSetlists()}
}
function updateSaveButton(){if(!state.current){$("saveBtn").disabled=true;$("saveBtn").textContent="Adicionar";return}const exists=state.setlist.some(x=>x.entryId&&x.entryId===state.current.entryId);$("saveBtn").disabled=exists;$("saveBtn").textContent=exists?"Adicionada":"Adicionar"}
